...
( function( $ ){
		...
		apollo13framework_font_icons_selector = function(){
			...
			if(icons_box.length){
				var $body = $(document.body),
					icons = icons_box.children(),
					current_input,

					show_icons_box = function(){
						current_input = $(this);
						...
						$body.on('click.iconSelector', hide_verify);
					},

					hide_icons_box = function(){
						current_input = null;
						icons_box.hide();
						$body.off('click.iconSelector');
					},
					...
					hide_verify = function(e){
						if(typeof e.target !== 'undefined'){
							var check = $(e.target);
							if(check.is(current_input) || check.is(icons_box) || check.parents('#a13-fa-icons').length){
								//current_input.focus();
							}
							else{
								hide_icons_box();
					...
					fill_input = function(){
						current_input.val($(this).attr('title')).trigger('input'); //no focus, cause it creates risk that it will not refresh
					};

				icons_box.prependTo('#customize-controls');

				$body.on('focus', 'input.a13-fa-icon', {}, show_icons_box);
			...
			var frame, header,
				...
				customizer_walk = function () {
					...
					if(notice_enabled === 1){
						...
						if(typeof api.panel('section_header_settings') !== 'undefined' && api.panel('section_header_settings').expanded()){
			...
			api.bind( 'ready', function() {
				...
				api.previewer.bind('synced', function () {
					...
					if(frame.length){
						...
						api.state.unbind('change', customizer_walk);
						api.state.bind('change', customizer_walk);
					}
				});
			});
		},
	...
	api.bind( 'ready', function() {
		...
		api.control(G.options_name+'[custom_css]').container.find( '.customize-control-title:first' ).addClass( 'screen-reader-text' );
	});
