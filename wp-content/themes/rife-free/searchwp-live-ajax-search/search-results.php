...
<!--		--><?php //$post_type = get_post_type_object( get_post_type() ); echo esc_html( $post_type->labels->singular_name ); ?>
		<div class="searchwp-live-search-result">
			<a href="<?php echo esc_url( get_permalink() ); ?>">
					<?php if(has_post_thumbnail()){
						...
						echo '<span class="icon" style="background-image: url('.esc_attr($src).');"></span>';
					}
					else{
						?><span class="icon fa fa-file-text-o"></span><?php
					}?>
				<span class="title"><?php the_title(); ?></span>
				<?php echo apollo13framework_posted_on(); ?>
	...
	?>
	<a href="<?php echo esc_url( apollo13framework_current_url().'?s='.$search ); ?>" class="all-results"><?php esc_html_e('View all results', 'rife-free'); ?></a>
<?php else : ?>
