...
function apollo13framework_apollo13_info() {
	...
	echo '<p>'.esc_html__( 'You can check out what is new in change log or just move on with your usual work.', 'rife-free' ).
	     ' <a href="http://www.apollo13.eu/themes_update/apollo13_framework_theme/index.html#change-log">'.esc_html__( 'View change log.', 'rife-free').'</a>'.
	     '</p>';

	include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
	if( !is_plugin_active( 'apollo13-framework-extensions/apollo13-framework-extensions.php' ) ){
		$tgm = TGM_Plugin_Activation::get_instance();
		echo '<p>'.esc_html__( 'Some features(like Albums, Works, shortcodes, plugins support) of Rife Theme requires an additional plugin before you will be able to use it. ', 'rife-free' ).
		     ' <a class="button button-primary" href="'.esc_url( $tgm->get_tgmpa_url() ).'">'.esc_html__( 'Install plugin', 'rife-free').'</a>'.
		     '</p>';
	}
	...
	echo '<p>'.esc_html__( 'If you have existing page, then you can start from scratch by changing theme options.', 'rife-free' ).
	     ' <a class="button" href="'.esc_url( admin_url( 'customize.php') ).'">'.esc_html__( 'Go to Customizer', 'rife-free').'</a>'.
	     '</p>';

	echo '<p>'.esc_html__( 'If you have existing page, you can also try to import one of our designs to speed up your work. You will have to do it without demo data.', 'rife-free' ).
...
function apollo13framework_apollo13_export() {
	apollo13framework_theme_pages_header();
	apollo13framework_get_demo_exporter_content();
	apollo13framework_theme_pages_footer();
}
...
function apollo13framework_apollo13_help() {
	...
	<h3 class="center"><a href="https://apollo13themes.com/contact/"><?php echo esc_html__( 'Contact Apollo13 Themes', 'rife-free' ); ?></a></h3>
	<p><?php
...
<div class="wrap apollo13-page <?php echo esc_attr( $current_subpage ); ?> about-wrap">
	...
	<div class="wp-badge"><?php echo esc_html__( 'Version', 'rife-free' ).' '.esc_html( A13FRAMEWORK_THEME_VER ); ?></div>
