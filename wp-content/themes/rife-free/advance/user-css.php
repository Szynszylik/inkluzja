...
//global sidebars
	 ...
	 *  logo
	 */
	$logo_image        = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'logo_image' ), 'url(%s)' );
	$logo_image_2x     = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'logo_image_high_dpi' ), 'url(%s)' );
	...
	if ( $logo_type === 'image' ) {
		...
		if ( is_array( $_logo_image ) && isset( $_logo_image['width'] ) ) {
			//desktop
			...
			$bonus_space                      = 20;  //20px on each side of centered logo to breath from menu
			if ( $apollo13framework_a13->get_option( 'logo_with_shield' ) === 'on' ) {
				$bonus_space += $logo_shield_padding;
			}
			$logo_max_width_half_desktop = (float) ( $temp_width / 2 ) + $bonus_space . 'px';
	...
	$logo_image_hover        = (int) $apollo13framework_a13->get_option( 'logo_image_opacity' );
...
//border
	...
	$header_sticky_logo_image                = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_sticky_logo_image' ), 'url(%s)' );
	$header_sticky_logo_image_2x             = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_sticky_logo_image_high_dpi' ), 'url(%s)' );
	...
	$header_light_logo_image                = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_light_logo_image' ), 'url(%s)' );
	$header_light_logo_image_2x             = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_light_logo_image_high_dpi' ), 'url(%s)' );
	...
	$header_dark_logo_image                = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_dark_logo_image' ), 'url(%s)' );
	$header_dark_logo_image_2x             = apollo13framework_make_css_rule( 'background-image', $apollo13framework_a13->get_option_media_url( 'header_dark_logo_image_high_dpi' ), 'url(%s)' );
...
//filter
	...
	$blog_bricks_max_width = apollo13framework_make_css_rule( 'max-width', $apollo13framework_a13->get_option( 'blog_bricks_max_width' ), '%spx' );
	$blog_margin           = $apollo13framework_a13->get_option( 'blog_brick_margin' ) . 'px';
//space in case of different layout mode
	$blog_layout_mode        = $apollo13framework_a13->get_option( 'blog_layout_mode' );
	$blog_item_bottom_gutter = '';
	if ( $blog_layout_mode === 'fitRows' ) {
		$blog_item_bottom_gutter = 'margin-bottom: ' . $blog_margin . ';';
	}
...
//filter for album bricks
	...
	$albums_bricks_max_width = apollo13framework_make_css_rule( 'max-width', $apollo13framework_a13->get_option( 'albums_list_bricks_max_width' ), '%spx' );
	$albums_margin           = $apollo13framework_a13->get_option( 'albums_list_brick_margin' ) . 'px';
//space in case of different layout mode
	$albums_layout_mode        = $apollo13framework_a13->get_option( 'albums_list_layout_mode' );
	$albums_item_bottom_gutter = '';
	if ( $albums_layout_mode === 'fitRows' ) {
		$albums_item_bottom_gutter = 'margin-bottom: ' . $albums_margin . ';';
	}
...
//filter
	...
	$works_bricks_max_width = apollo13framework_make_css_rule( 'max-width', $apollo13framework_a13->get_option( 'works_list_bricks_max_width' ), '%spx' );
	$works_margin           = $apollo13framework_a13->get_option( 'works_list_brick_margin' ) . 'px';
//space in case of different layout mode
	$works_layout_mode        = $apollo13framework_a13->get_option( 'works_list_layout_mode' );
	$works_item_bottom_gutter = '';
	if ( $works_layout_mode === 'fitRows' ) {
		$works_item_bottom_gutter = 'margin-bottom: ' . $works_margin . ';';
	}
	 ...
	 *  People list
	 */
	$people_bricks_max_width = apollo13framework_make_css_rule( 'max-width', '1920', '%spx' );
	$people_margin           = '10' . 'px';
//space in case of different layout mode
	$people_layout_mode        = 'fitRows';
	$people_item_bottom_gutter = '';
	if ( $people_layout_mode === 'fitRows' ) {
		$people_item_bottom_gutter = 'margin-bottom: ' . $people_margin . ';';
	}
...
//menu hover effect
	...
	if ( strlen( $menu_hover_effect ) && ! in_array( $menu_hover_effect, $ignored_effects ) ) {
		//prepare file system
		$can_process_file = true;
		$url              = wp_nonce_url( 'user-css.php', '' );
		if ( false === ( $creds = request_filesystem_credentials( $url, '', false, false, null ) ) ) {
			$can_process_file = false; // stop processing here
		}
		if ( ! WP_Filesystem( $creds ) ) {
			request_filesystem_credentials( $url, '', true, false, null );
			$can_process_file = false;
		}

		if ( $can_process_file ) {
			$menu_hover_effect_css_file = get_theme_file_path( 'css/menu-effects/' . $menu_hover_effect . '.css' );
			if ( $wp_filesystem->exists( $menu_hover_effect_css_file ) ) {
				$user_css .= $wp_filesystem->get_contents( $menu_hover_effect_css_file );
			}
		}
...
.header-variant-one_line_centered .menu-cell .top-menu{
	width: -webkit-calc(50% - $logo_max_width_half_desktop);
...
@media only screen and (max-width: 1024px) {
	...
	.header-variant-one_line_centered .menu-cell .top-menu{
		width: -webkit-calc(50% - $logo_max_width_half_mobile);
...
.albums-bricks{
	$albums_bricks_max_width
}
.albums-grid-container{
	margin-right: -$albums_margin;
}
html[dir=\"rtl\"] .albums-grid-container{
	margin-right: 0;
	margin-left: -$albums_margin;
}
...
.albums-columns-4 .grid-master{
	width: -webkit-calc(25% - $albums_margin);
	width:         calc(25% - $albums_margin);
}
.albums-columns-4 .archive-item.w2{
	width: -webkit-calc(50% - $albums_margin);
	width:         calc(50% - $albums_margin);
}
.albums-columns-4 .archive-item.w3{
	width: -webkit-calc(75% - $albums_margin);
	width:         calc(75% - $albums_margin);
}

/* 3 columns */
.albums-columns-3 .archive-item,
.albums-columns-3 .grid-master{
	width: -webkit-calc(33.3333333% - $albums_margin);
	width:         calc(33.3333333% - $albums_margin);
}
.albums-columns-3 .archive-item.w2{
	width: -webkit-calc(66.6666666% - $albums_margin);
	width:         calc(66.6666666% - $albums_margin);
}
...
.albums-columns-4 .archive-item.w4{
	width: -webkit-calc(100% - $albums_margin);
	width:         calc(100% - $albums_margin);
}
...
.works-bricks{
	$works_bricks_max_width
}
.works-grid-container{
	margin-right: -$works_margin;
}
html[dir=\"rtl\"] .works-grid-container{
	margin-right: 0;
	margin-left: -$works_margin;
}
...
.works-columns-4 .grid-master{
	width: -webkit-calc(25% - $works_margin);
	width:         calc(25% - $works_margin);
}
.works-columns-4 .archive-item.w2{
	width: -webkit-calc(50% - $works_margin);
	width:         calc(50% - $works_margin);
}
.works-columns-4 .archive-item.w3{
	width: -webkit-calc(75% - $works_margin);
	width:         calc(75% - $works_margin);
}

/* 3 columns */
.works-columns-3 .archive-item,
.works-columns-3 .grid-master{
	width: -webkit-calc(33.3333333% - $works_margin);
	width:         calc(33.3333333% - $works_margin);
}
.works-columns-3 .archive-item.w2{
	width: -webkit-calc(66.6666666% - $works_margin);
	width:         calc(66.6666666% - $works_margin);
}
...
.works-columns-4 .archive-item.w4{
	width: -webkit-calc(100% - $works_margin);
	width:         calc(100% - $works_margin);
}
...
/* ==================
   ...
   ==================*/
.people-bricks{
	$people_bricks_max_width
}
.people-grid-container{
	margin-right: -$people_margin;
}
html[dir=\"rtl\"] .people-grid-container{
	margin-right: 0;
	margin-left: -$people_margin;
}
...
.people-columns-4 .grid-master{
	width: -webkit-calc(25% - $people_margin);
	width:         calc(25% - $people_margin);
}

/* 3 columns */
.people-columns-3 .archive-item,
.people-columns-3 .grid-master{
	width: -webkit-calc(33.3333333% - $people_margin);
	width:         calc(33.3333333% - $people_margin);
}

/* 2 columns */
.people-columns-2 .archive-item,
.people-columns-2 .grid-master{
	width: -webkit-calc(50% - $people_margin);
	width:         calc(50% - $people_margin);
}

/* 100% width bricks */
.people-columns-1 .grid-master,
.people-columns-1 .archive-item{
	width: -webkit-calc(100% - $people_margin);
	width:         calc(100% - $people_margin);
}
...
.posts-columns-3 .grid-master{
	width: -webkit-calc(33.3333333% - $blog_margin);
	width:         calc(33.3333333% - $blog_margin);
}
.posts-columns-3 .archive-item.w2{
	width: -webkit-calc(66.6666666% - $blog_margin);
	width:         calc(66.6666666% - $blog_margin);
}
...
.woocommerce-page .shop-columns-4 ul.products .grid-master{
	width: -webkit-calc(25% - $shop_margin);
	width:         calc(25% - $shop_margin);
}
.woocommerce-page .shop-columns-3 ul.products li.product,
.woocommerce-page .shop-columns-3 ul.products .grid-master{
	width: -webkit-calc(33.3333333% - $shop_margin);
	width:         calc(33.3333333% - $shop_margin);
}
.woocommerce-page .shop-columns-2 ul.products li.product,
.woocommerce-page .shop-columns-2 ul.products .grid-master{
	width: 50%;
	width: -webkit-calc(50% - $shop_margin);
...
.products-list .layout-full_padding.with-sidebar .content-box{
	margin-left: $shop_margin;
	width: -webkit-calc(75% - $shop_margin);
...
@media only screen and (min-width: 1560px) {
	.products-list .layout-full.with-sidebar .content-box{
		width: -webkit-calc(100% - 320px - $shop_margin); /* 320 sidebar*/
		width: 		   calc(100% - 320px - $shop_margin);
	}
}
...
@media only screen and (max-width: 1400px) and (min-width: 1025px) {
	/* make sure that sidebar wont get too narrow */
	.products-list .layout-full_padding.with-sidebar .content-box{
		width: -webkit-calc(70% - $shop_margin);
