...
if(!function_exists('apollo13framework_make_media_collection')){
	 ...
	 * @param null|int $id post ID
	 */
    function apollo13framework_make_media_collection($id = null){
	    if(is_null($id)){
	        $id = get_the_ID();
	    }
	    ...
	    <ul class="gallery-media-collection screen-reader-text">
			<?php echo apollo13framework_prepare_frontend_gallery_html( apollo13framework_prepare_gallery_attachments( $value ), $order, $id ); ?>
		</ul>
	    <?php
    }
}
 ...
 */
function apollo13framework_prepare_frontend_gallery_html( $attachments, $order, $id ){
	global $apollo13framework_a13;
	...
	if ( $attachments ) {
		...
		$brick_proportion   = get_post_meta( $id, '_bricks_proportions_size', true);
 ...
 * */
				...
				if ( $item['id'] === 'external' ) {
					...
					$collector[ 'video_player' ]   = apollo13framework_get_movie_link( $temp );       //iframe address
			...
			apollo13framework_frontend_gallery_item_html( $collector, $show_desc );
 ...
 *
 * @param array $collector    item settings useful to generate HTML
 * @param bool  $show_desc    should description of item be displayed
 */
function apollo13framework_frontend_gallery_item_html( $collector, $show_desc ){
	global $apollo13framework_a13;
	static $id = 0;
	$id++;
	$proofing_enabled = array_key_exists('proofing_checked', $collector);
	...
	if(strlen($collector[ 'filter_tags' ])){
		$album_filters = apollo13framework_get_media_filters();
		$item_filters = array();
		$data_filter = '';
 ...
 */
function apollo13framework_get_media_filters(){
	static $indexed_filters;
	
	//cache once collected filters
	if(!is_array($indexed_filters)){
		$id = get_the_ID();
		$value = get_post_meta( $id, '_images_n_videos' , true);
			...
			if ( $media_count ) {
				...
				if(count($filters)){
					...
					foreach($filters as $key => $filter){
						$indexed_filters['filter_'.$key] = $filter;
					}
				}
			}
		}
	...
	return $indexed_filters;
}

/**
 * Prints out filter for media in single album
 */
function apollo13framework_print_media_filters(){
	$filters = apollo13framework_get_media_filters();
	...
	if ( count( $filters ) ){
		...
		foreach ( $filters as $term_id => $term_name ) {
			echo '<li data-filter="' . esc_attr( $term_id ). '"><a href="#' . esc_url( $term_id ) . '">' . esc_html( $term_name ). '</a></li>';
		}
 ...
 */
function apollo13framework_print_proofing_filters(){
	$proofing_finished = get_post_meta( get_the_ID(), '_proofing_finished', true );
