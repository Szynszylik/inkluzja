 ...
 */
add_filter( 'wp_image_editors', 'a13ir_wp_image_editor' );

function a13ir_wp_image_editor() {
    return array(
 ...
 */
add_action( 'admin_init', 'a13ir_wp_image_editor_check' );

function a13ir_wp_image_editor_check() {
    $arg = array( 'mime_type' => 'image/jpeg' );
    if ( wp_image_editor_supports( $arg ) !== true ) {
        add_filter( 'admin_notices', 'a13ir_wp_image_editor_check_notice' );
    }
}

function a13ir_wp_image_editor_check_notice() {
    printf( "<div class='error'><p>%s</div>",
        esc_html__( 'The server does not have ImageMagick or GD installed and/or enabled! Any of these libraries are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing.', 'rife-free' ) );
}
...
if ( ! is_admin() || $ajax_for_vc_grid ) {
	add_filter( 'image_resize_dimensions', 'a13ir_image_resize_dimensions', 10, 5 );
}

if ( ! function_exists( 'a13ir_image_resize_dimensions' ) ) {
	function a13ir_image_resize_dimensions( /** @noinspection PhpUnusedParameterInspection */ $payload, $orig_w, $orig_h, $dest_w, $dest_h, /** @noinspection PhpUnusedParameterInspection */ $crop = false ) {
...
add_filter( 'image_downsize', 'a13ir_image_downsize', 1, 3 );


function a13ir_image_downsize( /** @noinspection PhpUnusedParameterInspection */ $out, $id, $size ) {
    if ( ! is_array( $size ) ) {
        return false;
    }
