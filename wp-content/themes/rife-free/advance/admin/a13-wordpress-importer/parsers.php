...
class WXR_Parser {
	function parse( $file ) {
		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'rife-free' ) . '</strong><br />';
			echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'rife-free' ) . '</p>';
		}
	...
	class WP_FileSystem_Credentials {
		static function check_credentials() {
			...
			if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
				return new WP_Error( 'XML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'rife-free' ) );
			}
	...
	class WXR_Parser_SimpleXML {
		function parse( $file ) {
			...
			if ( ! $success || isset( $dom->doctype ) ) {
				return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'rife-free' ), libxml_get_errors() );
			}
			...
			if ( ! $wxr_version ) {
				return new WP_Error( 'WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'rife-free' ) );
			}
