...
require get_theme_file_path( 'advance/admin/a13-wordpress-importer/parsers.php' );
...
class Apollo13Framework_Import extends WP_Importer {
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'rife-free' ) . '</strong><br />';
			echo esc_html__( 'The file does not exist, please try again.', 'rife-free' ) . '</p>';
			die();
		}
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import category %s', 'rife-free' ), esc_html($cat['category_nicename']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import post tag %s', 'rife-free' ), esc_html($tag['tag_name']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( esc_html__( 'Failed to import %1$s %2$s', 'rife-free' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				echo '<br />';
				continue;
			}
		}
	...
	function process_posts() {
		...
		$memory_taken = $delta = $max_delta = 0;
		$max_memory_to_use = $this->return_bytes( ini_get('memory_limit') );
		...
		//Elementor not doing slashing on every import since 2.1.0
		$wp_slash_elementor = (defined('ELEMENTOR_VERSION') && version_compare(ELEMENTOR_VERSION, '2.1.0', '>=') && version_compare(ELEMENTOR_VERSION, '2.1.1', '<='));
		...
		foreach ( $this->posts as $post ) {
			...
			if($how_long > $this->max_time_running ){
				...
				printf( esc_html__( 'Importing posts for %s seconds.', 'rife-free' ), esc_html( round( $how_long, 1 ) ) );
				echo '<br />';
				$this->last_post_did = $post['post_id'];
				return false;
			}

			$memory_taken_previous = $memory_taken;
			$memory_taken = memory_get_usage();
			if( $memory_taken > 0 && $memory_taken_previous > 0){
				$max_delta = max($max_delta, $delta,( $memory_taken - $memory_taken_previous ) );
			}
			if( $memory_taken_previous > 0){
				$delta = $memory_taken - $memory_taken_previous;
			}

			if($memory_taken + (1 * $max_delta ) > $max_memory_to_use ){
				printf(
					/* translators: %1$s: memory taken, 2$s: memory limit, 2$s: memory delta */
					esc_html__( 'Already used %1$s out of %2$s bytes of memory. Max delta was %3$s. Starting over to not exceed memory limits.', 'rife-free' ),
					esc_html( $memory_taken ),
					esc_html( $max_memory_to_use ),
					esc_html( $max_delta )
				);
				echo '<br />';
				$this->last_post_did = $post['post_id'];
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				/* translators: %1$s: post_title, 2$s: post_type */
				printf( esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'rife-free' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				/* translators: %1$s: post type singular name, 2$s: post_title */
				printf( esc_html__('%1$s &#8220;%2$s&#8221; already exists.', 'rife-free'), esc_html( $post_type_object->labels->singular_name ), esc_html( $post['post_title'] ) );
				echo '<br />';
...
//				else
				...
				if ( is_wp_error( $post_id ) ) {
					/* translators: %1$s: post type singular name, 2$s: post_title */
					printf( esc_html__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'rife-free' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( esc_html__( 'Failed to import %1$s %2$s', 'rife-free' ), esc_html($taxonomy), esc_html($term['name']) );
							echo '<br />';
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			esc_html_e( 'Menu item skipped due to missing menu slug', 'rife-free' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			...
			printf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'rife-free' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				esc_html__( 'Fetching attachments is not enabled', 'rife-free' ) );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', esc_html__('Invalid file type', 'rife-free') );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
					'import_file_error',
					sprintf(
						/* translators: %1$d: error code, %2$s: error code name, %3$s: URL */
						__('Remote server returned %1$d %2$s for %3$s', 'rife-free'),
						$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
			...
			return new WP_Error( 'import_file_error', esc_html__('Remote file is incorrect size', 'rife-free') );
		}
		...
		if ( 0 == $filesize ) {
			...
			return new WP_Error( 'import_file_error', esc_html__('Zero size file downloaded', 'rife-free') );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error( 'import_file_error', sprintf( esc_html__('Remote file is too large, limit is %s', 'rife-free'), size_format($max_size) ) );
		}
