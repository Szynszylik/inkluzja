<?php
//prepare file system
function apollo13framework_demo_data_prepare_filesystem(){
	...
	if ( $wp_filesystem->exists( $demo_config_file ) ) {
		...
		//backup plan - special case when there's no google api key in demo data
		if( !isset( $apollo13framework_site_config['plugins_configs']['wpgmza_google_maps_api_key'] ) ){
			$apollo13framework_site_config['plugins_configs']['wpgmza_google_maps_api_key']  = 'AIzaSyA2a2tu7HNPEFEVAd6vzc6qSeRrqY6Qc1c';
		}
	}
 ...
 * */
function apollo13framework_demo_data_start( $demo_id ) {
	$response = wp_remote_post( A13FRAMEWORK_IMPORT_SERVER . '/api.php',
		array(
			'method'  => 'POST',
			'timeout' => 15,
			'body'    => array(
...
function apollo13framework_demo_data_download_files( $sublevel, &$sublevel_name, $demo_id ) {
	...
	if ( $file_to_download == '' && $revo_to_download == '' ) {
		...
		if ( is_writable( A13FRAMEWORK_IMPORTER_TMP_DIR .'/'.$demo_id ) ) {
			...
			$response      = wp_remote_get(
				A13FRAMEWORK_IMPORT_SERVER . '/download.php',
				array(
					'timeout'    => 300,
					'stream'     => true,
			...
			return array( 'sublevel' => false, 'response' => array( 'code'=> 1013,'message' => A13FRAMEWORK_IMPORTER_TMP_DIR . '/' .$demo_id . '/' . $path . ' - ' . esc_html__( 'this folder has incorrect permissions set, the script is unable to write a file into it. Import stopped - please contact your hosting admin or Apollo13 support team.', 'rife-free' ) ) );
		}
...
function apollo13framework_demo_data_clear_content() {
	global $wpdb;
	...
	if ( class_exists( 'YITH_WCWL_Install' ) ) {
		...
		$path        = apollo13framework_get_plugin_basename_from_slug( 'yith-woocommerce-wishlist', $all_plugins );
...
function apollo13framework_demo_data_install_plugins( $sublevel, &$sublevel_name, $demo_id, $chosen_options ) {
	...
	if ( strlen( $sublevel ) === 0 ) {//we will install first plugin on list but in second call of this function
		...
		apollo13framework_do_plugin_install( $plugins[ $sublevel ] );
...
function apollo13framework_demo_data_install_plugins_2() {
	$demo = unserialize(get_option(A13FRAMEWORK_TPL_SLUG .'_processed_demo_import'));
	$plugins = $demo['plugins'];
	...
	$this_plugin = current( $plugins );
	if ( $this_plugin['slug'] === 'yith-woocommerce-wishlist' ) {
		apollo13framework_do_plugin_install( current( $plugins ) );
	}
...
function apollo13framework_get_plugin_basename_from_slug( $slug, &$plugins ) {
 ...
 * @return bool
 */
function apollo13framework_do_plugin_install( $plugin = array() ) {
	...
	$plugin['file_path'] = apollo13framework_get_plugin_basename_from_slug( $plugin['slug'], $installed_plugins );
	...
	/** @noinspection PhpIncludeInspection */
	require_once get_theme_file_path( 'advance/admin/class-a13-plugin-installer-skin.php' ); // Overwrite of Plugin_Installer_Skin that doesn't clear buffer
	...
	$title = sprintf( esc_html__( 'Installing %s', 'rife-free' ), $plugin['name'] );
	...
	if ( $install_type === 'update' ) {
		...
		/* translators: %s: plugin name */
		echo sprintf( esc_html__( 'Plugin updated: %s', 'rife-free' ), esc_html( $plugin['name'] ) ) . "\r\n";
	} elseif ( $install_type === 'install' ) {
		$upgrader->install( $source );
		/* translators: %s: plugin name */
		echo sprintf( esc_html__( 'Plugin installed: %s', 'rife-free' ), esc_html( $plugin['name'] ) ) . "\r\n";
	}
	...
	if ( $activate_plugin ) {
		...
		if ( is_wp_error( $activate ) ) {
			...
			/* translators: %s: plugin name */
			echo sprintf( esc_html__( 'Plugin activated: %s', 'rife-free' ), esc_html(  $plugin['name'] ) ) . "\r\n";
		}
	}
...
function apollo13framework_demo_data_install_revo_sliders($demo_id) {
...
function apollo13framework_demo_data_install_content( $sublevel, &$sublevel_name, $demo_id, $chosen_options ) {
	...
	if ( class_exists( 'Apollo13Framework_Import' ) ) {
		...
		if ( ! $is_done ) {
			...
			$sublevel        = esc_html__( 'While importing content...', 'rife-free' );
			/* translators: %s: post ID */
			$sublevel_name   = sprintf( esc_html__( 'Imported %1$d of %2$d items', 'rife-free' ), $last_item, $all_items ) . "\r\n";
...
function apollo13framework_demo_data_setup_widgets() {
...
function apollo13framework_demo_data_setup_fp() {
	global $apollo13framework_site_config;
	if ( is_array($apollo13framework_site_config) ) {
...
function apollo13framework_demo_data_setup_plugins_configs() {
	global $apollo13framework_site_config;
	if ( is_array($apollo13framework_site_config) ) {
...
function apollo13framework_demo_data_setup_wc() {
	global $apollo13framework_site_config;
	if ( is_array($apollo13framework_site_config) ) {
...
function apollo13framework_demo_data_setup_menus() {
	global $apollo13framework_site_config;
	if ( is_array($apollo13framework_site_config) ) {
...
function apollo13framework_demo_data_setup_permalinks() {
	global $wp_rewrite;
...
function apollo13framework_demo_data_import_predefined_set( $demo_id ) {
	/* @var $wp_filesystem WP_Filesystem_Base */
...
function apollo13framework_demo_data_generate_custom_style() {
	global $apollo13framework_a13;
...
function apollo13framework_demo_data_clean($demo_id){
	$demo = unserialize(get_option(A13FRAMEWORK_TPL_SLUG .'_processed_demo_import'));
...
return apollo13framework_demo_data_prepare_filesystem();