...
//Lets decide what is the title
	if ( is_search() ) {
		...
		$title = sprintf( esc_html__( '%1$d Search results for "%2$s"', 'rife-free' ), $count, get_search_query() );
	} elseif ( is_archive() ) {
		if ( is_author() ) {
			/* translators: %s - author name */
			$title = sprintf( esc_html__( 'Author Archives: %s', 'rife-free' ), "<span class='vcard'>" . get_the_author() . "</span>" );
		} elseif ( is_category() ) {
			/* translators: %s - category name */
			$title = sprintf( esc_html__( 'Category Archives: %s', 'rife-free' ), '<span>' . single_cat_title( '', false ) . '</span>' );
		} elseif ( is_tag() ) {
			/* translators: %s - tag name */
			$title = sprintf( esc_html__( 'Tag Archives: %s', 'rife-free' ), '<span>' . single_tag_title( '', false ) . '</span>' );
		} elseif ( is_day() ) {
			/* translators: %s - day */
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'rife-free' ), '<span>' . get_the_date() . '</span>' );
		} elseif ( is_month() ) {
			/* translators: %s - month */
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'rife-free' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );
		} elseif ( is_year() ) {
			/* translators: %s - year */
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'rife-free' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
		} else {
			$title = esc_html__( 'Blog Archives', 'rife-free' );
		}
	}
	...
	<article id="content" class="clearfix">
		<div class="content-limiter">
			<div id="col-mask">
				...
				<?php the_posts_pagination(); apollo13framework_result_count(); ?>
